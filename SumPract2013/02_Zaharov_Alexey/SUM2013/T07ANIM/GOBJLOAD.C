 ...
 */
VOID LoadMaterials( az1GOBJ *Go, CHAR *FileName )
{
  FILE *F;
  az1MATERIAL DefMat, Mat;
  BOOL IsFirst = TRUE;
...
BOOL AZ1_GeomLoad( az1GOBJ *Go, CHAR *FileName )
{
  ...
  AZ1_PrimDefaultColor = VecSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT
    ...
    AZ1_PrimCreate(&prim, AZ1_PRIM_TRIMESH, vn, fn * 3, NULL, NULL);
    /* копируем вершины */
    ...
    AZ1_GeomAddPrim(Go, &prim);
  }

  free(ReadV);
  return TRUE;
} /* End of 'AZ1_GeomLoad' function */
