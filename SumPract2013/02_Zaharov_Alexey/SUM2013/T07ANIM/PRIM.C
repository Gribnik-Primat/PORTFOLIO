...
VEC AZ1_PrimDefaultColor = {1, 1, 1};
                     ...
                     VERTEX *Vert, INT *Ind )
{
  ...
  memset(Prim, 0, sizeof(az1PRIM));
  if (Type == AZ1_PRIM_GRID)
  ...
  if (Vert == NULL)
    for (i = 0; i < nv; i++)
      Prim->V[i].C = AZ1_PrimDefaultColor;
  else
    /* копируем вершины */
  ...
  return TRUE;
} /* End of 'AZ1_PrimCreate' function */
                          ...
                          VEC Loc, VEC Du, VEC Dv )
{
  ...
  if (!AZ1_PrimCreate(Prim, AZ1_PRIM_GRID, W, H, NULL, NULL))
    return FALSE;
  ...
  return TRUE;
} /* End of 'AZ1_PrimCreatePlane' function */
                           ...
                           VEC Loc, FLT Radius )
{
  ...
  return TRUE;
} /* End of 'AZ1_PrimCreateSphere' function */
 ...
 *   (BOOL) TRUE при успехе, FALSE при нехватке памати.
 */
BOOL AZ1_PrimCreateHeightField( az1PRIM *Prim, CHAR *FileName,
                                VEC Loc, VEC Du, VEC Dv )
{
  HBITMAP hBm, hBmDib;
  BITMAP Bm;
         ...
         IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE)) != NULL)
  {
          ...
          (VOID *)&Bits, NULL, 0)) != NULL)
    {
      ...
      /* создаем плоскость */
      if (AZ1_PrimCreatePlane(Prim, Bm.bmWidth, Bm.bmHeight, Loc, Du, Dv))
      {
        INT i, j;
  ...
  if (Prim->V == NULL)
    return FALSE;
  return TRUE;
} /* End of 'AZ1_PrimCreateHeightField' function */
