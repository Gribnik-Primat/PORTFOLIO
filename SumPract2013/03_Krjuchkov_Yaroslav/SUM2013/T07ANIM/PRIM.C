...
VEC IK1_PrimDefaultColor = {1, 1, 1};
                     ...
                     VERTEX *Vert, INT *Ind )
{
  ...
  memset(Prim, 0, sizeof(ik1PRIM));
  if (Type == IK1_PRIM_GRID)
  ...
  if (Vert == NULL)
    for (i = 0; i < nv; i++)
      Prim->V[i].C = IK1_PrimDefaultColor;
  else
    /* копируем вершины */
  ...
  return TRUE;
} /* End of 'IK1_PrimCreate' function */
                          ...
                          VEC Loc, VEC Du, VEC Dv )
{
  ...
  if (!IK1_PrimCreate(Prim, IK1_PRIM_GRID, W, H, NULL, NULL))
    return FALSE;
  ...
  return TRUE;
} /* End of 'IK1_PrimCreatePlane' function */
                           ...
                           VEC Loc, FLT Radius )
{
  ...
  return TRUE;
} /* End of 'IK1_PrimCreateSphere' function */
                                ...
                                VEC Loc, VEC Du, VEC Dv )
{
  ...
  else
  {
          ...
          (VOID *)&Bits, NULL, 0)) != NULL)
    {
      ...
      /* создаем плоскость */
      if (IK1_PrimCreatePlane(Prim, Bm.bmWidth, Bm.bmHeight, Loc, Du, Dv))
      {
        INT i, j;
        ...
        for (i = 0; i < Prim->GH - 1; i++)
          for (j = 0; j < Prim->GW - 1; j++)
          {
            ...
            Prim->V[n].N =
              VecSubVec(Prim->V[n + 1].P, Prim->V[n].P),
  ...
  if (Prim->V == NULL)
    return FALSE;
  return TRUE;
} /* End of 'IK1_PrimCreateHeightField' function */
