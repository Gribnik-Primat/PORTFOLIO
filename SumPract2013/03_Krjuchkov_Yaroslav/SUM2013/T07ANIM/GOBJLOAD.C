...
BOOL IK1_GeomLoad( ik1GOBJ *Go, CHAR *FileName )
{
  ...
  IK1_PrimDefaultColor = VecSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT
    ...
    IK1_PrimCreate(&prim, IK1_PRIM_TRIMESH, vn, fn * 3, NULL, NULL);
    /* копируем вершины */
    ...
    IK1_GeomAddPrim(Go, &prim);
  }

  free(ReadV);
  return TRUE;
} /* End of 'IK1_GeomLoad' function */
