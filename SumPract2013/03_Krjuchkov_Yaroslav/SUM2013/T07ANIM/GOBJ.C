...
VOID IK1_GeomClose( ik1GOBJ *Go )
{
  if (Go->Prims != NULL)
  {
    ...
    for (i = 0; i < Go->NumOfPrims; i++)
      IK1_PrimClose(&Go->Prims[i]);
    free(Go->Prims);
  }
  if (Go->Mats != NULL)
    free(Go->Mats);
  memset(Go, 0, sizeof(ik1GOBJ));
} /* End of 'IK1_GeomClose' function */
...
{
  ik1PRIM *newblock = malloc(sizeof(ik1PRIM) * (Go->NumOfPrims + 1));

  if (newblock == NULL)
    return FALSE;
  /* копируем старые примитивы */
  memcpy(newblock, Go->Prims, sizeof(ik1PRIM) * Go->NumOfPrims);
  ...
  return TRUE;
} /* End of 'IK1_GeomAddPrim' function */
...
{
  ik1MATERIAL *newblock = malloc(sizeof(ik1MATERIAL) * (Go->NumOfMats + 1));

  if (newblock == NULL)
    return FALSE;
  /* копируем старые примитивы */
  memcpy(newblock, Go->Mats, sizeof(ik1MATERIAL) * Go->NumOfMats);
...
VOID IK1_GeomDraw( ik1GOBJ *Go )
{
  ...
  for (i = 0; i < Go->NumOfPrims; i++)
  {
    ...
    /* рисуем примитив */
    if (i != 17)
      IK1_PrimDraw(&Go->Prims[i]);
    /*else
    {
      glBlendColor(1, 1, 1, 0.3);
      glBlendFunc(GL_SRC0_ALPHA, GL_CONSTANT_ALPHA);   
      IK1_PrimDraw(&Go->Prims[i]);
    } */
    glBindTexture(GL_TEXTURE_2D, 0);
  }
} /* End of 'IK1_GeomDraw' function */
