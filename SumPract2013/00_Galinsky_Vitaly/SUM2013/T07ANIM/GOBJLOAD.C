...
static VOID LoadMaterials( vg4GOBJ *Go, CHAR *FileName )
{
  ...
  _makepath(FullName, ModelDrive, ModelDir, FileName, "");
  ...
  while (fgets(Buf, sizeof(Buf), F) != NULL)
  {
    ...
    if (NumOfParts > 1)
      ...
      {
        _makepath(FullName, ModelDrive, ModelDir, Parts[1], "");
        strncpy(Mat.MapD, FullName, sizeof(Mat.MapD) - 1);
      }
