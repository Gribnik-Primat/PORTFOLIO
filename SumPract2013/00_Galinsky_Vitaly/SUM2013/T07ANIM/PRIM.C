                                ...
                                VEC Loc, VEC Du, VEC Dv )
{
  ...
  if (VG4_ImageLoad(&Bits, &W, &H, FileName))
  {
    /* создаем плоскость */
    if (VG4_PrimCreatePlane(Prim, W, H, Loc, Du, Dv))
    {
      INT i, j;
      ...
      for (i = 0; i < Prim->GH; i++)
        ...
        {
          INT n = i * Prim->GW + j;
          Prim->V[n].P = VecAddVec(Prim->V[n].P, VecMulNum(norm, ((Bits[n] >> 8) & 0xFF) / 10.0));
          Prim->V[n].C = VecSet(1, 1, 1); //VecSet(rand() / 32767.0, rand() / 32767.0, rand() / 32767.0);
        }
      ...
      for (i = 0; i < Prim->GH - 1; i++)
        for (j = 0; j < Prim->GW - 1; j++)
        {
          ...
          Prim->V[n].N =
            VecNormalize(VecCrossVec(VecSubVec(Prim->V[n + 1].P, Prim->V[n].P),
