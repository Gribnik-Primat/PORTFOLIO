...
export default class VolumeFilter3d {
  ...
  erasePixels(x_, y_, z_, size, depth, vDir, isothreshold, startflag, mouseup, normalmode) {
    ...
    m4.makeBasis(vB, vT, normal.multiplyScalar(-1));
    ...
    this.lastBackDistance.push(-Math.round(Math.abs(Math.tan(vDir.normalize().angleTo(normalGauss.normalize())))
      * (size)));
    this.lastNormalMode.push(normalmode);


    /*let centraldistance = 0;
    let eraseByThreshold = false;
    let zcentral = 0; //trying to move cylinder back by z axis (direction axis of cylinder)
    if (normalmode === false) { //some manipulatian with cylinder for tangential mode
      zcentral = 0 - Math.round(Math.abs(Math.tan(vDir.normalize().angleTo(normalGauss.normalize()))) * (size));
    ...
    for (zcentral; zcentral < depth + 1; zcentral++) {
     // measuring distance between top center of cylinder and first point
      const point = new THREE.Vector3(0, 0, zcentral);
    ...
    /*if (Math.abs(this.prevPos - targetX - targetY - targetZ) < radius) {
      for (let y = -rxy; y < rxy + 1; y++) { //beggining to deleting
      const rx = Math.sqrt(rxy*rxy-y*y);
    ...
    for (let a = 0; a < undoiterations; a++) {
      ...
      while (this.queue.length > 0) {
        ...
        if (Math.sqrt(RotPoint.x * RotPoint.x + RotPoint.y * RotPoint.y) > rxy ||
          Math.abs(RotPoint.z) > lastDepth || RotPoint.z < lastback) {
          continue;
        }
        for (let x = this.point.x - 1; x <= this.point.x + 1; x++) {
          for (let y = this.point.y - 1; y <= this.point.y + 1; y++) {
