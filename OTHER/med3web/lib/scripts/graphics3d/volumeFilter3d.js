...
export default class VolumeFilter3d {
  ...
  */
  ...
  scaleDown(pixelsSrc, xDimSrc, yDimSrc, zDimSrc, pixelsDst, xScale, yScale, zScale) {
...
//    this.yDimAO = 128;
//    this.zDimAO = 128;
//    this.xDimAO = Math.floor(Math.floor(this.xDim / 2));
//    this.yDimAO = Math.floor(Math.floor(this.yDim / 2));
//    this.zDimAO = Math.floor(Math.floor(this.zDim / 1));
...
/*
    this.tmpVolume = new Uint8Array(this.xDimAO * this.yDimAO * this.zDimAO);
    this.scaleDown(this.bufferTextureCPU, this.xDim, this.yDim, this.zDim, this.tmpVolume, 2, 2, 1);
	this.tmpVolumetex = new THREE.DataTexture3D(this.tmpVolume, this.xDimAO, this.yDimAO, this.zDimAO);
    this.tmpVolumetex.format = THREE.RedFormat;
    this.tmpVolumetex.type = THREE.UnsignedByteType;
    this.tmpVolumetex.wrapS = THREE.ClampToEdgeWrapping;
    this.tmpVolumetex.wrapT = THREE.ClampToEdgeWrapping;
    this.tmpVolumetex.wrapR = THREE.ClampToEdgeWrapping;
    this.tmpVolumetex.magFilter = THREE.LinearFilter;
    this.tmpVolumetex.minFilter = THREE.LinearFilter;
    this.tmpVolumetex.needsUpdate = true;
    const tmptexelSize = new THREE.Vector3(1.0 / this.xDimAO, 1.0 / this.yDimAO, 1.0 / this.zDimAO);
    matAO.create(this.tmpVolumetex, tmptexelSize, this.vectorsTex, numVertices, (mat) => {
*/
