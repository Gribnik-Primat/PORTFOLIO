...
cbuffer ConstantBuffer : register( b0 )
{
	...
	float4 vLightColor[9];
	float4 vOutputColor;
	float4 vLightRadius[9];
	float4 vLightPos[9];
}
...
float4 PS( PS_INPUT input) : SV_Target
{
	...
	for (int i = 0; i<2; i++)
	{
		...
		if(tmp > 0)
		{
			...
			// Special zeroing component for the task condition.
			L_specular = 0;
...
float4 PS(PS_INPUT input) : SV_Target
{
	float4 finalColor = 0;
	// InIntensity ratios of diffused light
	float Kd[9] = {0.8,1.4,1,1,1,1,1,1,1};
	// InIntensity ratios of specular light
	float Ks[9] = {2,2,2,2,2,2,2,2,0};
	float ns = 8;
	// Constnant intenstity of light indeoendent from raduius;
	float L_ambient[9] = {0,0,0,0,0,0,0,0,0};
